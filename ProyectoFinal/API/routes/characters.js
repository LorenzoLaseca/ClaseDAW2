...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (character == undefined) {
            return res.status(400).json(
                { error: `No existe el character con id ${req.params.id}` }
            )
        }
        res.json({
            error: null,
...
router.put('/inventory/:id', async (req, res) => {
    ...
    if (!inventoryitemDatabase) {
        return res.status(400).json(
            { error: `El inventory item con id ${id} no existe` }
        )
    }
...
router.delete('/:id/inventory/:itemid', async (req, res) => {
    ...
    try {
        const deleteResponse = await InventoryItem.deleteOne({ itemId: itemId, characterId:req.params.id});
        if (deleteResponse.deletedCount == 1) {
            res.json({
                error: null,
